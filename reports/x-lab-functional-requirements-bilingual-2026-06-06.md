...
| FR-042 | 需求管理 / Demand management | P0 | 需求管理表格页面 / Requirement management table page | 需求、状态、负责人、影响范围和验收标准需要一个可见、密集、可扫描的表格型工作台页面，而不是只存在于 Markdown 文档或卡片看板中。 / Requirements, status, owners, affected scope, and acceptance criteria need a visible, dense, scannable table-based workspace page instead of living only in Markdown documents or card-based boards. | `requirements-board.html` 必须用表格展示需求清单，字段包括 ID、优先级、模块、需求、状态、负责人、影响范围、验收标准和关联文件；不得用卡片作为需求清单主体；页面仍保留需求总览、改动登记字段、门禁规则、追溯矩阵、关联文件入口，并从首页 footer 和 B2B dashboard 进入。 / `requirements-board.html` must display the requirement list as a table with ID, priority, module, requirement, status, owner, affected scope, acceptance criteria, and linked files; cards must not be used as the main requirement list; the page still includes summary, change intake fields, gate rules, traceability matrix, linked file entries, and access from the home footer and B2B dashboard. |
