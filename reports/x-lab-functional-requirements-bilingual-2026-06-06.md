...
| FR-042 | 需求管理 / Demand management | P0 | 需求总览表页面 / Requirement overview table page | 需求、VOC、提出时间、计划完成时间、负责人、状态、影响范围、验收标准和测试验证情况需要集中在一个可见、密集、可扫描的总览表中，而不是分散在卡片、看板或多个区块里。 / Requirements, VOC, request date, planned completion date, owner, status, affected scope, acceptance criteria, and test validation status need to live in one visible, dense, scannable overview table instead of being scattered across cards, boards, or multiple sections. | `requirements-board.html` 暂时只保留需求总览表；表格字段必须包含 ID、优先级、模块、需求、关联 VOC、需求提出时间、计划完成时间、状态、负责人、影响范围、验收标准、测试验证情况和关联文件；页面不得使用卡片作为需求清单主体，也暂不展示改动登记、门禁规则、追溯矩阵等额外区块。 / `requirements-board.html` should temporarily keep only the requirement overview table; table fields must include ID, priority, module, requirement, related VOC, request date, planned completion date, status, owner, affected scope, acceptance criteria, test validation status, and linked files; cards must not be used as the main requirement list, and extra sections such as change intake, gate rules, and traceability matrix should not be shown for now. |
