...
- `x-lab-voc-database-sync` 自动化必须执行端到端 VOC 同步：检查并更新本地 `xlab-data.js` 与 `voc-list.html` 渲染数据、同步 Notion VOC/需求数据库、执行本地校验、提交变更、推送 GitHub `main`，并用非强推同步提交更新 `gh-pages`；如果任一环节失败，必须生成或更新 `ISS-*`。
...
- The `x-lab-voc-database-sync` automation must run end-to-end VOC sync: check and update local `xlab-data.js` plus `voc-list.html` rendering data, sync the Notion VOC and requirement databases, run local validation, commit changes, push GitHub `main`, and update `gh-pages` with a non-force sync commit. If any step fails, create or update an `ISS-*` record.
...
| 日期 / Date | 需求编号 / Requirement ID | 修改内容 / Change | 原因 / Reason | 影响范围 / Affected Scope |
| --- | --- | --- | --- | --- |
| 2026-06-13 | FR-054 / FR-057 / FR-060 | 明确 `x-lab-voc-database-sync` 必须端到端同步本地 VOC 页面、Notion、GitHub main 和 gh-pages，并在失败时进入问题清单。 / Clarified that `x-lab-voc-database-sync` must sync the local VOC page, Notion, GitHub main, and gh-pages end-to-end, and must create an issue record when it fails. | 用户指出新 VOC 同步流程没有自动跑起来；原自动化只覆盖漂移检查，没有明确提交与 GitHub 发布动作。 / User noted the new VOC sync flow did not run automatically; the previous automation only covered drift checks and did not explicitly include commit and GitHub publishing actions. | `x-lab-voc-database-sync` automation, `xlab-data.js`, `voc-list.html`, Notion VOC database, GitHub `main` / `gh-pages`, issue list. |
